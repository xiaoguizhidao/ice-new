...
class GoMage_Feed_Helper_Generator extends Mage_Core_Helper_Abstract {
    ...
    {
        $dir = Mage::getBaseDir('media') . DS . 'productsfeed';
        if (!is_dir($dir)) {
            mkdir($dir, 755, true);
        }
    ...
    {
        $dir = Mage::getBaseDir('media') . DS . 'productsfeed' . DS . 'logs';
        if (!is_dir($dir)) {
            mkdir($dir, 0755, true);
        }
    ...
    public function getGenerateInfo($feed_id, $init = false) {
		...
		if (file_exists($file_path) && !$init) {
            //Mage::log('Load generate info file:' . $file_path, null, 'feed-cron.log', false);
