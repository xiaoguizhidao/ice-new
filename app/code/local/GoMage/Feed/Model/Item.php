...
class GoMage_Feed_Model_Item extends Mage_Core_Model_Abstract {
    ...
    public function getFileNameWithExt() {
        $filename = str_replace('{#date#}', date('Ymd'), $this->getFilename());
        $filename = str_replace('{#date#}', date('His'), $filename);
    ...
    public function getRootCategory() {
        if (is_null($this->_rootCategory)) {
            $store = $this->getStore();
            $this->_rootCategory = Mage::getModel('catalog/category')->load($store->getRootCategoryId());
        }
    ...
    public function getProductsCollection() {
        ...
        if (is_null($this->_productCollection) && $this->getId()) {
            ...
            if (in_array($this->getUseLayer(), array(GoMage_Feed_Model_Adminhtml_System_Config_Source_Uselayer::NO, GoMage_Feed_Model_Adminhtml_System_Config_Source_Uselayer::NO_WITH_CHILD))) {
                ...
                $collection->getSelect()->where('price_index.entity_id is not null');
            }
            ...
            if ($this->getFilter()) {
                ...
                $filter_values = Mage::helper('core')->jsonDecode($this->getFilter());
                $filters = array();
    ...
    /**
     * @var GoMage_Feed_Model_Mysql4_Custom_Attribute_Collection
     */
     ...
     * @return string
     */
    protected function _getAttributes(array $codes)
    {
        sort($codes);
        $cacheHash = md5(Mage::helper('core')->jsonEncode($codes));

        if (!isset($this->_attributes[$cacheHash])) {
            $this->_attributes[$cacheHash] = Mage::getModel('eav/entity_attribute')->getCollection()->setEntityTypeFilter(Mage::getResourceModel('catalog/product')->getEntityType()->getData('entity_type_id'))->setCodeFilter($codes);
        }

        return $this->_attributes[$cacheHash];
    }

    /**
     * @return GoMage_Feed_Model_Mysql4_Custom_Attribute_Collection
     */
    protected function _getCustomAttributes()
    {
        if (is_null($this->_customAttributes)) {
            $this->_customAttributes = Mage::getResourceModel('gomage_feed/custom_attribute_collection');
            $this->_customAttributes->load();
        }

        return $this->_customAttributes;
    }

    protected function _writeTempFile($collection)
    {
        ...
        if (is_dir($fileDir)) {
            if ($this->getData('type') == 'csv') {
                $delimiter = $this->_getCsvDelimeter();
                $enclosure = $this->_getCsvEnclosure();
                $mapping   = json_decode($this->getContent());
                $codes     = array();
                ...
                foreach ($customAttributes as $attribute) {
                    $options = Zend_Json::decode($attribute->getData('data'));
                    if ($options && is_array($options)) {
                        ...
                        $attribute->setOptions(array());
                    }
                }

                $attributes   = $this->_getAttributes($codes);
                ...
                foreach ($collection as $product) {
                    ...
                    $fields = array();
                    $this->prepareProductCategory($product, $rootCategory);

                    foreach ($mapping as $col) {
                        $value = null;
                        if ($col->type == 'attribute') {
                            if ($col->attribute_value) {
                                $value = $this->getProductAttributeValue($product, $col->attribute_value, $attributes, $customAttributes);
                            }
                            else {
                                $value = '';
                            }
                        } else if ($col->type == 'parent_attribute') {
                            if ($col->attribute_value) {
                                ...
                                if ($parentProduct->getId()) {
                                    if ($col->attribute_value == 'url') {
                                        ...
                                        $value = $parentProduct->getAttributeText($col->attribute_value);
                                    }

                                    if (empty($value)) {
                                        if ( $col->attribute_value == 'sku_amazon' ) {
                                            $value = $this->getProductAttributeValue($parentProduct, 'sku', $attributes, $customAttributes);
                                        } else {
                                            $value = $this->getProductAttributeValue($parentProduct, $col->attribute_value, $attributes, $customAttributes);
                                        }
                                    }
                                } else {
                                    if ($col->attribute_value == 'url') {
                            ...
                            // if empty
                        } else if ($col->type == 'if_empty_child_attribute') {
                            if ($col->attribute_value) {
                                $value = $this->getProductAttributeValue($product, $col->attribute_value, $attributes, $customAttributes);
                                if (empty($value)) {
                                    $parentProduct = $this->getParentProduct($product);
                        ...
                        } else if ($col->type == 'if_empty_parent_attribute') {
                            if ($col->attribute_value) {
                                $value = $this->getProductAttributeValue($product, $col->attribute_value, $attributes, $customAttributes);
                                if(empty($value)){
                                    ...
                                    if($child_product){
                                        if ($child_product->getId()) {
                                            $value = $this->getProductAttributeValue($child_product, $col->attribute_value, $attributes, $customAttributes);
                                        }
                                    }
                                }
                            } else {
                        ...
                        if ($col->prefix_type == 'attribute') {
                            if ($col->attribute_prefix_value) {
                                $valuePrefix = $product->getAttributeText($col->attribute_prefix_value);
                                if (empty($valuePrefix)) {
                                    $valuePrefix = $this->getProductAttributeValue($product, $col->attribute_prefix_value, $attributes, $customAttributes);
                                }
                            } else {
                                $valuePrefix = '';
                            }
                        } else if ($col->prefix_type == 'parent_attribute') {
                            if ($col->attribute_prefix_value) {
                                $parentProduct = $this->getParentProduct($product);
                                if ($parentProduct->getId()) {
                                    if ($col->attribute_prefix_value == 'url') {
                                        ...
                                        $valuePrefix = $parentProduct->getAttributeText($col->attribute_prefix_value);
                                    }

                                    if (empty($valuePrefix)) {
                                        if ($col->attribute_prefix_value == 'sku_amazon') {
                                            $valuePrefix = $this->getProductAttributeValue($parentProduct, 'sku', $attributes, $customAttributes);
                                        } else {
                                            $valuePrefix = $this->getProductAttributeValue($parentProduct, $col->attribute_prefix_value, $attributes, $customAttributes);
                                        }
                                    }
                                } else {
                                    if ($col->attribute_prefix_value == 'url') {
                            ...
                            $valuePrefix = $col->prefix_value;
                        }
                        // suffix value
                        if ($col->suffix_type == 'attribute') {
                            if ($col->attribute_suffix_value) {
                                $valueSuffix = $product->getAttributeText($col->attribute_prefix_value);
                                if (empty($valueSuffix)) {
                                    $valueSuffix = $this->getProductAttributeValue($product, $col->attribute_suffix_value, $attributes, $customAttributes);
                                }
                            } else {
                                $valueSuffix = '';
                            }
                        } else if ($col->suffix_type == 'parent_attribute') {
                            if ($col->attribute_suffix_value) {
                                $parentProduct = $this->getParentProduct($product);
                                if ($parentProduct->getId()) {
                                    if ($col->attribute_suffix_value == 'url') {
                                        ...
                                        $valueSuffix = $parentProduct->getAttributeText($col->attribute_suffix_value);
                                    }
                                    if (empty($valueSuffix)) {
                                        if ($col->attribute_suffix_value == 'sku_amazon') {
                                            $valueSuffix = $this->getProductAttributeValue($parentProduct, 'sku', $attributes, $customAttributes);
                                        } else {
                                            $valueSuffix = $this->getProductAttributeValue($parentProduct, $col->attribute_suffix_value, $attributes, $customAttributes);
                                        }
                                    }
                                } else {
                                    if ($col->attribute_suffix_value == 'url') {
                                        ...
                                        $valueSuffix = $product->getAttributeText($col->attribute_suffix_value);
                                    }
                        ...
                        $fields[] = $valuePrefix. $this->getDelimiterPrefix(). $value . $this->getDelimiterSufix() . $valueSuffix;
                    }
    ...
    public function getProductAttributeValue(&$product, $coll_attribute_value, $attributes, $custom_attributes) {
        ...
        if (false !== strpos($value, Mage::getStoreConfig(Mage_Core_Model_Store::XML_PATH_SECURE_BASE_URL))) {
            $value = str_replace(Mage::getStoreConfig(Mage_Core_Model_Store::XML_PATH_SECURE_BASE_URL), Mage::getStoreConfig(Mage_Core_Model_Store::XML_PATH_UNSECURE_BASE_URL), $value);
        }
    ...
    {
        $rootBlock = Mage::getModel('gomage_feed/item_block', array('content' => $this->getContent(), 'feed' => $this));
        @file_put_contents($this->_getFilePath(), $rootBlock->render());
    }

    /**
     * @return string
     */
    protected function _getCsvDelimeter()
    {
     ...
     * @return string
     */
    protected function _getCsvEnclosure()
    {
     ...
     */
    protected function _goToPage(Mage_Catalog_Model_Resource_Product_Collection $collection, $page)
    {
        $pageSize = intval($this->getIterationLimit());
        if (!$pageSize) {
            $pageSize = 50;
        }
        Mage::log('Initialize collection limitation:' . $pageSize . ' '.  ($page * $pageSize), null, 'feed-cron.log', false);
        $collection->clear()->getSelect()->limit($pageSize, $page * $pageSize);
    ...
    /**
     * @return $this
     * @throws GoMage_Feed_Exception_Feed_Stopped
     * @throws GoMage_Feed_Exception_Feed_Suspend
     */
    protected function _generateCsv()
    {
        $currentPage  = $this->_generateInfo->getCurrentPage();
        $collection   = $this->getProductsCollection();

        $this->_generateInfo->setData('total_records', $collection->getSize())->save();
        $maxAllowedMemory = Mage::helper('gomage_feed')->getMaxAllowedMemory() * 0.8;

        while (count($this->_goToPage($collection, $currentPage++))) {
            $this->_initGenerateInfo(true);
            $this->_writeTempFile($collection);
            $this->_generateInfo->setCurrentPage($currentPage);
            $this->_generateInfo->addGeneratedRecords(count($collection))->save();
            if ($this->_generateInfo->getData('stopped')) {
                throw new GoMage_Feed_Exception_Feed_Stopped();
            }
            Mage::log('Memory usage:' . memory_get_usage() . ' of ' . $maxAllowedMemory, null, 'feed-cron.log', false);
            if (memory_get_usage() > $maxAllowedMemory) {
                throw new GoMage_Feed_Exception_Feed_Suspend('Memory limit has been reached');
            }
        }

        $csvData  = @file_get_contents($this->getTempFilePath());
        $fileDir  = Mage::helper('gomage_feed/generator')->getBaseDir();
        ...
        if ($this->getShowHeaders()) {
            $fields = array();
            $mapping = Mage::helper('core')->jsonDecode($this->getContent());
            foreach ($mapping as $col) {
                $fields[] = $col->name;
            }
            if ($this->getEnclosure() != 4) {
                fputcsv($fp, $fields, $this->_getCsvDelimeter(), $this->_getCsvEnclosure());
            } else {
                $this->extended_fputcsv($fp, $fields, $this->_getCsvDelimeter());
            }
        }
    ...
    /** @var  GoMage_Feed_Model_Generator */
    protected $_generateInfo;

    /**
     * @param bool $forceLoad
     * @return GoMage_Feed_Model_Generator
     */
    protected function _initGenerateInfo($forceLoad = false)
    {
        if ($forceLoad || is_null($this->_generateInfo)) {
            if ($forceLoad) {
                /** @var GoMage_Feed_Model_Generator $generateInfo */
                $generateInfo = Mage::helper('gomage_feed/generator')->getGenerateInfo($this->getId());
                Mage::log('Force load info:' . $this->getId(), null, 'feed-cron.log', false);
            } else {
                /** @var GoMage_Feed_Model_Generator $generateInfo */
                $generateInfo = Mage::helper('gomage_feed/generator')->getGenerateInfo($this->getId());
                if ($generateInfo->isSuspend()) {
                    Mage::log('Wake up suspended process:' . $this->getId(), null, 'feed-cron.log', false);
                } else {
                    Mage::log('Initialize new generation process:' . $this->getId(), null, 'feed-cron.log', false);
                    $generateInfo = Mage::helper('gomage_feed/generator')->getGenerateInfo($this->getId(), true);
                }
            }


            $this->_generateInfo = $generateInfo;
        }
    ...
    {
        Mage::log('Start generation:' . $this->getId(), null, 'feed-cron.log', false);
        $this->_initGenerateInfo();
        if (!$this->_generateInfo->isSuspend()) {
            if (file_exists($this->getTempFilePath())){
                Mage::log('Clear old tmp file:' . $this->getId(), null, 'feed-cron.log', false);
                unlink($this->getTempFilePath());
            }
        }
        $this->_generateInfo->start()->save();

        try {
            if ($this->getData('type') == 'csv') {
                Mage::log('Generate csv:' . $this->getId(), null, 'feed-cron.log', false);
                $this->_generateCsv();
            } else {
                Mage::log('Generate xml:' . $this->getId(), null, 'feed-cron.log', false);
                $this->_generateXml();
            }
            $this->setData('generated_at', date('Y-m-j H:i:s', time()));
            $this->save();
            Mage::log('FINISHED:' . $this->getId(), null, 'feed-cron.log', false);
            $this->_generateInfo->finish();
        } catch (GoMage_Feed_Exception_Feed_Suspend $e) {
            Mage::log('SUSPEND:' . $e->getMessage(), null, 'feed-cron.log', false);
            $this->_generateInfo->suspend()->save();
            throw $e;
        } catch (GoMage_Feed_Exception_Feed_Stopped $e) {
            Mage::log('STOPPED:' . $e->getMessage(), null, 'feed-cron.log', false);
            $this->_generateInfo->stop();
        } catch (Exception $e) {
            Mage::log('EXCEPTION:' . $e->getMessage(), null, 'feed-cron.log', false);
            $this->_generateInfo->stop();
        }
        $this->_generateInfo->save();
