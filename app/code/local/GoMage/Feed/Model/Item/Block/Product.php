...
class GoMage_Feed_Model_Item_Block_Product extends GoMage_Feed_Model_Item_Block {
    ...
    public function writeFeedTempXml(Mage_Catalog_Model_Resource_Product_Collection $collection)
    {
        $xmlProductBlockTemplateTmpPath = Mage::helper('gomage_feed/generator')->getBaseDir() . DS . 'feed-' . $this->getFeed()->getId() . '-xml-product-block-template.tmp';
        if (is_file($xmlProductBlockTemplateTmpPath) && is_readable($xmlProductBlockTemplateTmpPath)) {
            $content = @file_get_contents($xmlProductBlockTemplateTmpPath);
        } else {
            $content = '';
        }

        $templateAttributes = $this->getAllVars($content);

        $codes = array();

        foreach ($templateAttributes as $attributeCode) {
            $codes[] = $attributeCode;
        }

        $customAttributes = Mage::getResourceModel('gomage_feed/custom_attribute_collection');
        $customAttributes->load();

        foreach ($customAttributes as $_attribute) {
        ...
        foreach ($collection as $_product) {
            ...
            foreach ($templateAttributes as $attributeCode) {
                ...
                if (strpos($attributeCode, 'parent:') === 0) {
                    $attribute_code_parent = trim(str_replace('parent:', '', $attributeCode));
                    $parent_product = $this->getFeed()->getParentProduct($_product);
                    if ($parent_product->getId()) {
                        $value = $this->getFeed()->getProductAttributeValue($parent_product, $attribute_code_parent, $attributes, $customAttributes);
                    }
                    else {
                        $value = $this->getFeed()->getProductAttributeValue($_product, $attribute_code_parent, $attributes, $customAttributes);
                    }
                }
                else {
                    $value = $this->getFeed()->getProductAttributeValue($_product, $attributeCode, $attributes, $customAttributes);
                }
