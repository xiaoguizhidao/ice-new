...
class Mage_Paygate_Model_Authorizenet extends Mage_Payment_Model_Method_Cc
{
    ...
    const RESPONSE_REASON_CODE_ERROR_OCCURRED = 122;
    ...
    protected function _voidCardTransaction($payment, $card)
    {
        ...
        switch (true) {
            case self::RESPONSE_CODE_APPROVED == $result->getResponseCode()
                || self::RESPONSE_CODE_ERROR == $result->getResponseCode()
                    && self::RESPONSE_REASON_CODE_ERROR_OCCURRED == $result->getResponseReasonCode()
                    && false !== strpos($result->getData('response_reason_text'), 'Only transactions pending settlement can be voided'):
                if (self::RESPONSE_REASON_CODE_ERROR_OCCURRED == $result->getResponseReasonCode()
                    || self::RESPONSE_REASON_CODE_APPROVED == $result->getResponseReasonCode()
                    || self::RESPONSE_REASON_CODE_ALREADY_VOIDED == $result->getResponseReasonCode()) {
                ...
                $exceptionMessage = $this->_wrapGatewayError($result->getResponseReasonText());
                break;
            case self::RESPONSE_CODE_DECLINED == $result->getResponseCode():
            case self::RESPONSE_CODE_ERROR == $result->getResponseCode():
    ...
    protected function _postRequest(Varien_Object $request)
    {
        ...
        $r = new Varien_Object(explode(self::RESPONSE_DELIM_CHAR, $responseBody));
